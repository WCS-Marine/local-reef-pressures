...
#' @param bcu_top_threats Top threats for a single BCU, from \code{data/bcus_top_threats.rds}
#'
...
#' bcu_top_threats <- readRDS(here::here("data", "bcus_top_threats.rds"))[[bcu_name]]
#'
#' plot_threat_ranking_percentiles(bcu_percentiles, bcu_top_threats)
plot_threat_ranking_percentiles <- function(bcu_percentiles, bcu_top_threats) {

  bcu_top_threats <- bcu_top_threats %>%
  ...
  bcu_percentiles_combined <- bcu_percentiles %>%
    dplyr::inner_join(bcu_top_threats, by = c("bcu", "indicator"), suffix = c("", "_mean"))
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::geom_errorbar(ggplot2::aes(x = reorder(indicator_label, rank), y = percentile, ymin = percentile, ymax = percentile), data = bcu_top_threats) +
    ggplot2::scale_colour_gradient2(
      low = low_blue, mid = mid_yellow, high = high_red,
      midpoint = 0.5, limits = c(0, 1)
    ) +
    ggplot2::labs(x = NULL, y = "Threat Percentile",
         title = "Threats ranked from highest to lowest; BCU average and pixels compared to all reef pixels",
