...
ggplot2::theme_update(axis.title = element_blank(),
...
data %>%
  ...
  dplyr::summarise(fishing = median(grav_NC,na.rm=T),
...
ggplot2::theme_update(plot.title = element_text(hjust = 0.5, size = 10),
                      plot.margin = margin(0.2,0.2,0.2,0.2,"cm"),
...
ehe.arranged <- ehe %>% dplyr::arrange(Region) %>% dplyr::mutate(x.order=c(1:83))
...
a %>% dplyr::select(BCU_name, fishing, coast_dev, industr_dev, tourism, sediments, nitrogen, x.order, Region, Region_wrap) %>% 
