...
bcus <- allreefs %>%
  filter(is.bcu == "BCUs") %>%
  select(bcu = BCU_name, num_ports = num_ports_raw, grav_NC = grav_NC_raw, pop_count = pop_count_raw, reef_value = reef_value_raw, sediment = sediment_raw, nutrient = nutrient_raw, score, scoreth, scorepfc, scorecy, scoretr, scorecn, geometry)
